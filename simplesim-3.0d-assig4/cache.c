...
void next_line_prefetcher(struct cache_t *cp, md_addr_t addr) {
  ...
  // address for prefetcher
  md_addr_t next_addr = addr + cp -> bsize;

  // the next block's address is not in cache CP
  if(cache_probe(cp, next_addr) == 0){
    // get the initial address of block
...
void update_rpt_table(rpt_table_entry *rpt_entry, int new_stride){
  ...
  switch(rpt_entry -> rpt_entry_state){
    case INIT:
      if(stride_condition){
        rpt_entry -> rpt_entry_state = STEADY;
      }else{
        rpt_entry -> rpt_entry_state = TRANSIENT;
    ...
    case TRANSIENT:
      if(stride_condition){
        ...
        rpt_entry -> rpt_entry_state = NO_PRED;
        rpt_entry -> stride = new_stride;
      }
      break;
    case STEADY:
      if(stride_condition){
        ...
        rpt_entry -> rpt_entry_state = INIT;
      }
      break;
    case NO_PRED:
      if(stride_condition){
...
void stride_prefetcher(struct cache_t *cp, md_addr_t addr) {
  ...
  int rpt_table_index = (pc >> 3) % num_rpt_entries;
  // tag not matching: update entry
  if(cp -> rpt_table[rpt_table_index].tag != pc){
    cp -> rpt_table[rpt_table_index].tag = pc;
    cp -> rpt_table[rpt_table_index].prev_addr = addr;
    cp -> rpt_table[rpt_table_index].stride = 0;
    cp -> rpt_table[rpt_table_index].rpt_entry_state = INIT;
